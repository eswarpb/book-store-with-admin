...
const PlaceOrder = () => {
    ...
    <section className='max-padd-container'>
      <form onSubmit={onSubmitHandler} className='pt-28'>
        <div className='flex flex-col xl:flex-row gap-20 xl:gap-28 '>
          ...
          <div>
            <input onChange={OnChangeHandler} value={formData.zipcode} type='text' name='zipcode' placeholder='Zipcode' className='ring-1 ring-slate-900/15 p-1 pl-3 rounded-sm bg-primary outline-none w-1/2' required/>
  ...
  <div
    ...
    className={`${
      method === 'stripe' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'
    } px-4 py-2 rounded-lg text-xs cursor-pointer border border-gray-300 transition-all`}
  >
    Stripe
  </div>
  ...
  <div
    ...
    className={`${
      method === 'cod' ? 'bg-violet-900 text-white' : 'bg-gray-200 text-gray-800'
    } px-4 py-2 rounded-lg text-xs cursor-pointer border border-gray-300 transition-all`}
  >
    Cash on Delivery
  </div>
